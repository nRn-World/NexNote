...
import {
  Plus, Trash2, File as FileIcon, X, Code, Play,
  Camera, Pin, History as HistoryIcon, Tag as TagIcon,
  Sparkles, Clock, Copy, Check, Maximize2
} from 'lucide-react';
...
export default function App() {
  ...
  useEffect(() => {
    const handler = (e: KeyboardEvent) => {
      ...
      if (e.key === 'Escape') { setActiveNoteId(null); setShowHistory(false); }
    };
  ...
  const createNote = () => {
    if (!user) return;
    const n: Note = { id: uuidv4(), uid: user.uid, title: '', content: '', attachments: [], createdAt: Date.now(), updatedAt: Date.now() };
    handleSave(n);
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    for (let i = 0; i < files.length; i++) {
      ...
      try {
        ...
        addToast(`Kunde inte ladda upp ${file.name}.`, 'error');
      }
    }
  ...
  const toggleCodeEditor = () => {
    ...
    if (note.code) {
      setConfirm({
        open: true, title: 'Ta bort kod', danger: true,
        message: 'Är du säker på att du vill ta bort all kod?',
        onConfirm: () => { setConfirm(c => ({ ...c, open: false })); updateActiveNote({ code: undefined }); },
      });
    } else {
      updateActiveNote({ code: { html: '', css: '', js: '' } });
  ...
  const capturePreview = async () => {
    ...
    try {
      ...
      addToast('Kunde inte ta skärmdump. Vissa resurser kan blockeras av webbläsaren.', 'error');
    } finally {
  ...
  const handleAiAction = async (action: 'summarize' | 'fix') => {
    ...
    if (!apiKey || apiKey === 'MY_GEMINI_API_KEY') {
      addToast('Lägg till VITE_GEMINI_API_KEY i .env för att använda AI.', 'error'); return;
    }
    try {
      ...
      if (action === 'summarize') {
        addToast('Sammanfattning klar — se nedan.', 'info');
        alert('Sammanfattning:\n\n' + text);
      } else {
        updateActiveNote({ content: text });
        addToast('Text förbättrad med AI.', 'success');
      }
    } catch {
      addToast('Kunde inte köra AI-tjänsten.', 'error');
    } finally {
  ...
  if (!isAuthReady) {
    return <div className="flex h-screen w-full items-center justify-center bg-zinc-50 text-zinc-500">Laddar...</div>;
  }
    ...
    return (
      <div className="flex h-screen w-full items-center justify-center bg-zinc-50">
        <div className="bg-white p-8 rounded-xl shadow-sm border border-zinc-200 max-w-sm w-full text-center">
          ...
          <button onClick={signInWithGoogle} className="w-full py-2 px-4 bg-zinc-900 text-white rounded-md hover:bg-zinc-800 transition-colors">
            Logga in med Google
          </button>
        </div>
      </div>
        ...
        {activeNote ? (
          <>
            <NoteHeader
              ...
              onAiFix={() => handleAiAction('fix')}
              onAiSummarize={() => handleAiAction('summarize')}
            ...
            <div className="flex-1 overflow-y-auto p-4 md:p-8 lg:px-12 max-w-4xl mx-auto w-full flex flex-col relative">
              ...
              {/* History panel */}
              {showHistory && (
                <div className="absolute right-4 top-4 w-64 bg-white border border-zinc-200 rounded-xl shadow-xl z-30 flex flex-col max-h-[80%] overflow-hidden">
                  <div className="p-3 border-b border-zinc-100 flex items-center justify-between bg-zinc-50">
                    ...
                    <button onClick={() => setShowHistory(false)} className="text-zinc-400 hover:text-zinc-900"><X size={16} /></button>
                  </div>
                  <div className="flex-1 overflow-y-auto p-2 space-y-2">
                    {(!activeNote.history || activeNote.history.length === 0) ? (
                      ...
                      <div key={i} className="p-2 rounded-md border border-zinc-100 hover:bg-zinc-50 group transition-colors">
                        ...
                        <button onClick={() => restoreVersion(v)} className="w-full py-1 bg-zinc-100 text-[10px] font-bold uppercase rounded hover:bg-zinc-900 hover:text-white transition-all opacity-0 group-hover:opacity-100">
                          Återställ
                        </button>
                      </div>
                    ))}
              ...
              <div className="relative pl-10 md:pl-12 flex-1 flex flex-col">
                ...
                <div className="relative flex items-start group mb-2">
                  <button onClick={handleCopyTitle} className="absolute -left-10 md:-left-12 top-1/2 -translate-y-1/2 p-2 text-zinc-300 hover:text-zinc-600 opacity-0 group-hover:opacity-100 transition-all rounded-md hover:bg-zinc-100" title="Kopiera titel">
                ...
                <div className="relative flex-1 flex flex-col group">
                  <button onClick={handleCopyContent} className="absolute -left-10 md:-left-12 top-0 p-2 text-zinc-300 hover:text-zinc-600 opacity-0 group-hover:opacity-100 transition-all rounded-md hover:bg-zinc-100 z-10" title="Kopiera text">
          ...
          <div className="flex-1 flex-col items-center justify-center text-zinc-400 p-8 hidden md:flex">
            ...
            <button onClick={createNote} className="mt-6 px-4 py-2 bg-zinc-900 text-white rounded-md hover:bg-zinc-800 transition-colors flex items-center gap-2">
              <Plus size={18} /> Skapa ny anteckning
            </button>
            <p className="text-xs text-zinc-300 mt-3">Ctrl+N för ny anteckning</p>
          </div>
        )}
      </div>
